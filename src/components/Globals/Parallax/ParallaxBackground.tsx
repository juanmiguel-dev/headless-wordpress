...
export default function ParallaxBackground() {
    ...
    return (
        <div className="fixed inset-0 pointer-events-none z-[-1] overflow-hidden opacity-20">
            ...
            <motion.div
                style={{ y: y1, rotate }}
                className="absolute top-[10%] left-[5%] w-96 h-96 border border-white/40 rounded-full"
            />

            {/* Ornament 2: Floating code snippet vibes */}
            <motion.div
                style={{ y: y2 }}
                className="absolute top-[40%] right-[10%] font-mono text-xs text-white/40 select-none hidden md:block"
            >
                <pre>{`
  const architect = {
    stack: ['WP', 'Next.js'],
    performance: 'optimized'
        ...
        `}</pre>
            ...
            <motion.div
                style={{ y: y3 }}
                className="absolute bottom-[20%] left-[10%] w-64 h-64 border-l border-b border-white/40"
            />
