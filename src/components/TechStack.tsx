...
const TechStack = () => {
    ...
    return (
        <section className="pt-32 pb-12 bg-black text-white relative border-t border-white/40 overflow-hidden">
            {/* Background Texture */}
                ...
                <div className="flex flex-col md:flex-row items-end justify-between gap-12 mb-32">
                    <div className="max-w-2xl">
                        ...
                        <h2 className="text-5xl md:text-8xl font-black uppercase tracking-tighter leading-[0.8]">
                            Technical<br />
                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-neon-lime to-white/50 italic font-serif lowercase font-light ml-4">stack.</span>
                        </h2>
                    </div>
                    <p className="text-gray-500 font-light max-w-sm text-right text-sm leading-relaxed mb-4 border-r border-neon-lime/70 pr-8">
                        Engineering scalable digital ecosystems with a focused, performance-first technical architecture.
                    </p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-0 border-l border-t border-white/40">
                    {stack.map((item, index) => (
                        <div
                            key={index}
                            className="tech-card group p-10 border-r border-b border-white/40 bg-white/[0.03] hover:bg-white/[0.03] transition-all duration-500 relative overflow-hidden"
                        >
                            ...
                            <div className="flex justify-between items-start mb-12">
                                <span className="text-neon-lime/70 font-mono text-[10px] tracking-widest">{item.id}</span>
                                <div className="w-2 h-2 rounded-full border border-white/50 group-hover:bg-neon-lime group-hover:border-neon-lime transition-colors duration-500" />
                            </div>
                            ...
                            <ul className="space-y-4">
                                {item.tools.map((tool, idx) => (
                                    <li key={idx} className="flex items-center text-gray-400 text-xs font-mono group-hover:text-gray-300 transition-colors">
                                        <span className="w-1 h-1 bg-white/50 rounded-full mr-3 group-hover:bg-neon-lime/50" />
                                        {tool}
                                    </li>
                                ))}
                            </ul>
                            ...
                            {/* Decorative Ornament */}
                            <div className="mt-16 pt-8 border-t border-white/40 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                                <span className="text-[8px] font-mono text-white/40 uppercase tracking-[0.3em]">0x{index.toString(16).padStart(2, '0')} // NODE_ACTIVE</span>
                            </div>
                        </div>
                    ))}
                </div>
