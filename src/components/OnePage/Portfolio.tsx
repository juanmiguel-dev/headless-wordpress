...
export default function Portfolio({ posts }: PortfolioProps) {
    ...
    useGSAP(() => {
        ...
        if (featuredCards.length > 0) {
            // "Active System" pulse effect for featured cards
            ...
            // Scanning line for featured cards
            ...
            <div ref={containerRef} className="container mx-auto px-6 relative z-10">
                ...
                <div className="grid gap-12 md:grid-cols-2 lg:grid-cols-2">
                    {posts.map((post, index) => {
                        const isFeatured = post.categories?.nodes.some(
                        ...
                        return (
                            <motion.div
                                ...
                                className={`group relative flex flex-col bg-white/[0.02] border ${isFeatured ? 'border-neon-lime/30 featured-card' : 'border-white/5'} rounded-sm overflow-hidden`}
                            >
                                {isFeatured && (
                                    <>
                                        {/* Featured Pulse Layer */}
                                        <div className="featured-glow absolute inset-0 border border-neon-lime/40 pointer-events-none z-10" />
                                        <div className="scan-line absolute top-0 left-0 w-full h-[1px] bg-neon-lime/20 pointer-events-none z-10" />

                                        {/* Featured Badge */}
                                        <div className="absolute top-0 right-10 z-20">
                                            <div className="bg-neon-lime text-black font-mono text-[8px] font-bold px-3 py-1 uppercase tracking-widest shadow-[0_0_15px_rgba(223,255,0,0.3)]">
                                                Featured_Project
                                            </div>
                                        </div>
                                    </>
                                ...
                                <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-white/[0.01]">
                                    <div className="flex items-center gap-4">
                                        <span className={`text-[10px] font-mono ${isFeatured ? 'text-neon-lime' : 'text-neon-lime/50'} uppercase tracking-widest`}>
                                            {isFeatured ? 'PRIORITY_ALPHA' : `ID: ${post.id.slice(0, 8)}`}
                                        </span>
                                    ...
                                    <div className="flex gap-1">
                                        <div className={`w-1 h-1 rounded-full ${isFeatured ? 'bg-neon-lime/80 shadow-[0_0_5px_currentColor]' : 'bg-neon-lime/40'}`} />
                                ...
                                <div className="flex flex-col md:flex-row flex-1 min-h-[350px]">
                                    ...
                                    <div className="relative w-full md:w-1/2 overflow-hidden bg-black border-r border-white/5">
                                        {post.featuredImage?.node?.sourceUrl ? (
                                            <Image
                                                ...
                                                unoptimized={true}
                                                className={`object-cover ${isFeatured ? 'opacity-80 grayscale-0' : 'opacity-60 grayscale'} transition-all duration-1000 group-hover:scale-110 group-hover:grayscale-0 group-hover:opacity-100`}
                                            />
                                        ) : (
                                    ...
                                    <div className="p-10 flex-1 flex flex-col justify-between relative">
                                        {/* Glitch Title */}
                                        <div>
                                            <h3 className={`text-3xl font-black uppercase tracking-tighter text-white mb-6 leading-[0.9] transition-colors duration-300 ${isFeatured ? 'group-hover:text-neon-lime text-neon-lime/90' : 'group-hover:text-neon-lime'}`}>
                                                <Link href={`/${post.slug}`}>
                                                    {post.title}
                                                </Link>
                                            </h3>
                                            <div
                                                className="text-gray-400 text-sm font-light leading-relaxed mb-8 line-clamp-3"
                                        ...
                                        <div className="flex items-center justify-between mt-auto">
                                            ...
                                            <span className="text-[8px] font-mono text-white/10 uppercase tracking-widest hidden md:block select-none">
                                                {isFeatured ? 'STATUS: ACTIVE_ALPHA' : `NODE: 0x${index.toString(16).padStart(2, '0')}`}
                                            </span>
                                        </div>
